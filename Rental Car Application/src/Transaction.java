	...
	public int getDays(){
		...
		if((getPickYear() == getReturnYear()) && (getPickMonthValue() == getReturnMonthValue())) {
			if(getPickDate() == getReturnDate())
				...
				days = getReturnDate() - getPickDate()+1;
		}
...
////////////////////////////////////////////save all data to .txt file for the Owner//////////////////////////////
	...
	PrintWriter outFile = new PrintWriter(new FileOutputStream(new File("Transactions_Inventory.txt"), true));
		...
		outFile.println("License: "+aCustomer.getLicense());
        ...
        try{
            ...
            String sql="INSERT INTO CUSTOMER"+
                    "(FULL_NAME,AGE,LOCATION,CONTACT,LICENSE,VEHICLE,PICKDATE,RETURNDATE,PRICE)"+
                    "VALUES('"+aCustomer.getFullName()+"','"+getAge()+"','"+aCustomer.getLocation()+"','"+aCustomer.getContact()
                    +"','"+aCustomer.getLicense()+"','"+aCar.getCar()+"','"+getPickDate()+" "+getPickMonth()+" "+getPickYear()+"','"+
                    getReturnDate()+" "+getReturnMonth()+" "+getReturnYear()+"','"+getRentalPrice()+"')";
            s.execute(sql);
